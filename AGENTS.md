...
不同载体只负责各自范围，禁止在多个位置维护同一份完整需求：

1. 本文件中的产品不变量和工程规则是所有改动必须满足的硬约束。
2. `docs/specification.md` 是产品范围、外部行为和产品验收基线的权威来源。
3. `docs/architecture.md` 和已接受的 `docs/adr/` 记录工程边界与架构决策；ADR 只在
   明确声明替代关系时覆盖旧决策。
4. GitHub Issue 只定义单个任务的范围、任务特有验收条件、依赖和完成状态。Issue 必须
   引用相关规格章节，不得静默重新定义产品行为或架构约束。
5. `docs/roadmap.md` 只描述实施顺序和里程碑，不作为详细需求来源。

当这些来源出现矛盾时，停止实现并明确指出冲突。先在同一个变更中同步规格、架构文档
或 ADR 与相关 Issue，完成评审后再继续编码；不得猜测采用哪个版本。Issue 评论、PR 描述
和未合并讨论在写入上述权威来源前不构成新的项目约束。
