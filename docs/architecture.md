...
默认两个额外槽位时为 4、2、2。原始问题的覆盖候选按逐查询 reranker 排名选择；额外槽位有第二个名额时，
除 reranker 首位外还保留通道融合首位，若两者相同才回退到下一名 reranker。该双信号覆盖只处理
cross-encoder 与通道融合明显分歧的候选保留，不扩大配额或读取评测真值。再按候选的最高逐查询 reranker 分数融合排序并补足其余候选。该策略隐藏在
