...
新 Evaluation Report 的 observed evidence 同时记录 Chunk UUID 与既有文档版本/页码；旧 Report 的
Chunk UUID 可为空。诊断合并 query candidate 和 observed final evidence 的可靠映射，因此同页邻居
Chunk 被 Assessor 选中时仍可定位来源，不从文本或运行时数据库反推历史身份。
