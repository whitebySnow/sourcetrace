...
`retrieval` Service 为原始问题保留最多 4 个覆盖候选，且所有覆盖配额之和不超过实际 Top-K；
容量允许时，每个实际有候选的查询按输入顺序先保留一个，再优先补足原始问题，最后按额外槽位顺序补第二个候选，每槽至多 2 个。
默认两个额外槽位时为 4、2、2。再按候选的最高逐查询 reranker 分数融合排序并补足其余候选。该策略隐藏在
...
额外槽位的第二个覆盖位置以 reranker 第二名为默认候选。仅当该槽位的通道融合首位在逐查询
reranker 中仍处于有界分歧窗口内，才由其替换第二名；窗口为最终 Top-K 加上两个额外槽位最大
覆盖数，默认 Top-8 时为第 12 名。严重降级的通道首位不会挤掉 reranker 第二名，因此候选容量、
查询预算和 `4/2/2` 覆盖配额均不改变。
...
查询必须保留问题锚点，不得输出答案或结论。对于未显式归属的命名组件，规划器只能在稳定标题
提供可辩护方法或框架词项时，把该词项作为独立查询的词法锚点；该锚点只是检索假设，不表达
组件归属，也不构成回答证据。每个标题锚定槽位携带至少三个字符的 `title_anchor`，它必须是
去掉扩展名后的标题精确子串且在查询中出现。适配器拒绝把标题写成“属于”“来自”或等价英语
归属断言的查询，没有这种标题锚点时保持空计划。规划器的结构化调用固定使用 `temperature=0`，
多槽位计划进入独立的第二阶段：最终选中的每个槽位使用当前问题、最近用户问题、稳定标题列表
及自身第一阶段查询、文档标题和 `title_anchor` 并发细化。细化不能读取其他槽位结果，不能改变
标题或标题锚点、增加或合并槽位，且精化查询必须保留该锚点；无效细化槽位被丢弃。简单事实和
单槽反例不进入第二阶段。两个阶段都不得读取文档正文、
检索候选、答案或评测标签。提示词版本随运行轨迹保存。每条查询仍使用
Conversation 绑定的 Knowledge Base 重新执行向量检索。最终回答生成器接收用户原始问题和
本轮选中的文档证据，因此回答语言跟随原问题，Citation excerpt 保留文档原文。语言策略以
汉字数量与拉丁词数量的严格多数关系识别中文或英文问题，并把同一目标语言指令传给生成、
引用修复和声明支持度校验；结构化引用修复渲染与最终回答都按声明单元验证。语言不一致时安全
失败或拒答，服务端不自行翻译证据或
草稿。该门禁与证据支持度和 Citation 校验彼此独立，不能用语言匹配掩盖证据错误。检索查询只是
工具输入，不能作为证据。Retrieval Plan、实际执行查询与规划提示词版本写入 Answer Run，供
重放和评测使用。新运行还写入去敏的规划决策：初始计划 disposition、是否进行了唯一一次纠正、
初始槽位数，以及最终选中槽位的标题锚点和精化 disposition。初始规划阶段的供应商或结构失败写入 `failed`
disposition，同时运行保持失败而非被当作证据不足拒答。该记录不含模型原始 JSON、未选
查询、完整标题列表或提示词；迁移前运行保留 `null`，不能从当前配置反推其未知规划状态。
...
假设，同时不改变查询总预算、候选上限或证据门禁。Evidence Assessor 的候选接口直接复用
Retrieval Result 已有的文档标题和 PDF 页码，并从 Query Retrieval Result 计算每个 Chunk 实际
命中的 Retrieval Query；标题和页码是来源身份与位置元数据，命中查询只用于证据槽位分组，
不能作为回答证据。候选以稳定序号输入评估器；评估器在判定前按序逐项检查问题中的每个命名组件，
并保留直接支持任一组件的所有片段，不让一个来源的证据替代另一个命名来源。该步骤仍是受限的
模型判断，服务端不从标题、查询或评测真值推导支持关系。工作流不为此额外查询数据库。首次 Evidence Decision 判定不足但选中的片段由工作流保留，
补充检索后与新候选去重合并，并显式标记为上一轮已选；最终允许证据是此前选择与本轮选择的
稳定并集，但最终 `sufficient` 仍由本轮 Evidence Decision 决定。供应商返回与已执行 Retrieval
Query 重复的补充查询时，适配器只允许一次结构纠正，持续重复按非法响应安全失败。

当前回答决策由 `rag/workflow.py` 中的有界 LangGraph 深模块编排，对外只暴露
`AnswerWorkflow.run(request)` 事件流接口。固定节点为问题分析与有界查询规划、初始检索与
候选融合、结构化证据判断、可选的单次补充检索与重新融合、生成、初始引用校验与可选的单次
引用修复、声明支持度校验、最终引用校验、回答语言校验，以及回答或拒答终态。声明支持度校验是受限的模型
驱动语义门禁，逐项保留证据中的限定词和比较强度，
把混合声明拆为证据分别支持的最窄表述，再由服务端确定性渲染允许的引用；空声明、未知引用或
非法结构直接拒答。补充检索只能消耗 Answer Run 剩余的额外查询预算，并始终使用
...
状态。决策轨迹包含 Retrieval Plan 版本、去敏的初始计划和精化 disposition、实际执行查询及其
轮次、每条查询的候选 Chunk 原始排名与 cosine 相似度、RRF 配置与融合分数、reranker 的模型身份、revision、配置版本、候选
逐查询分数与重排排名、查询覆盖标记、最终重排排名、页面多样性选择结果、每次结构化证据判断
及所选 Chunk ID（包含跨轮保留后的有效并集）、证据评估器提出的有序补充查询、补充检索次数和
引用修复次数，使规划、融合、重排、补充检索与修复路径可以重放和评测。
每次引用校验还记录初稿或修复稿阶段、结构单元总数、引用数量，以及未引用或使用未知标签的
单元索引；这些诊断不得包含被拒绝的回答正文、完整提示词或文档内容。
引用修复适配器接收结构化声明与允许引用 ID，并在工作流再次执行声明支持度和引用校验前
确定性渲染每个声明单元的引用；自由文本修复不能直接成为最终回答。
OpenAI-compatible 供应商适配器把最终流式生成与内部结构化调用分成独立配置。DeepSeek 基线对
两类调用都显式关闭 thinking；不支持该扩展的兼容平台可配置为省略该字段。结构化调用必须满足
非空正文、`finish_reason=stop`、JSON 解码和精确业务 Schema；最终流只向下游发送
`delta.content`，忽略 keep-alive、reasoning content 和 usage-only chunk。只有明确的 `stop`
终态可进入引用校验，终态后的正文、`[DONE]` 前断连和未知终态都按安全供应商错误处理。瞬态
...
不复制问题、答案、提示词或文档正文。诊断区分已召回但未引用、同页不同
...
真实工作流新生成的 Evaluation Report observed evidence 同时记录 Chunk UUID 与既有文档版本/页码；
旧 Report 和不执行检索的 deterministic fake fixture 可为空，禁止为 fixture 伪造身份。诊断合并 query
candidate 和 observed final evidence 的可靠映射，因此同页邻居 Chunk 被 Assessor 选中时仍可定位
来源，不从文本或运行时数据库反推历史身份。
