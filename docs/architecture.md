...
最多增加一条反例查询，并且只用于主动搜索绝对化或否定性声明遗漏的限制或失败模式；简单事实、
对比、归属和组合题保持原始问题作为初始计划。证据不足后的单次补充检索可以使用剩余的一条
查询额度，因此一次 Answer Run 最多执行 2 条额外查询，规范化后重复的查询不执行。规划器的
结构化调用固定使用 `temperature=0`，提示词版本随运行轨迹保存。每条查询仍使用
