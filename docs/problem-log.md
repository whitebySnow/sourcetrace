...
## 38. Evidence Assessment 未完成逐候选闭环检查

**诊断**：Issue #81 的去敏轨迹已将两个 Retrieval 通过但拒答的 case 分为不同机制：`ARF-025`
在两轮中均未选择任何 Chunk，`ARF-026` 只保留 RAG 第 3 页的三个 Chunk，遗漏 ReAct 与
Self-RAG 的来源页。两者均不能由运行时读取 Dataset、预期页码或 case ID 修正；那样会把评测
真值泄漏进产品路径。

**缓解假设**：Evidence Assessor 的候选输入增加稳定序号，并在既有三字段结构化契约内明确要求
按序逐候选检查每个命名组件：每个直接支持任一组件的片段都必须选择，一个来源不能替代另一个
命名来源。该改动不增加供应商调用、候选数、Top-K、查询预算或任何 Evidence/Citation/Refusal
门禁；`evidence-assessment-v7` 记录其提示词语义。可控 HTTP fake 覆盖零选中、多来源漏选与
真正无直接支持时继续拒答的路径。

**验证边界**：离线测试只能锁定提示词和适配器契约，不能证明某个真实模型必然完成语义选择。
是否减少 `ARF-025`、`ARF-026` 的错误拒答，需要在代码审查和合并后，经单独授权运行只含这两题的
真实供应商探针；探针结果不得改写既有 30 题报告或泛化为产品准确率。

## 39. 动态多来源规划遗漏稳定标题锚点

**诊断**：Issue #89 的两题真实供应商探针中，`ARF-026` 在 Evidence Assessment 前已经检索失败。
动态规划把“环境动作”和“critique token”写成泛化中文查询，未复用知识库中 `ReAct`、`Self-RAG`
等稳定标题的可检索方法词；同一问题的版本化本地查询计划使用标题锚定的独立槽位时能够覆盖缺失
来源页。这是规划查询表达的缺口，不能据此归因于 embedding、reranker 或 Evidence Assessor。

**修复边界**：`two-stage-evidence-slots-v7` 允许规划器仅在稳定标题提供可辩护的方法或框架词项时，
将未显式归属的命名组件与该词项组成一个独立检索假设。标题仍不是证据，查询不得声称组件归属；
没有可辩护标题锚点时必须保持空计划。该改动不增加两条额外查询预算，不读取 Dataset、case ID、
预期证据或文档正文，也不改变融合、重排或 Evidence/Citation/Refusal 门禁。适配器还拒绝把标题
写成“属于”“来自”或等价英语归属断言的查询，避免模型违约时将检索假设伪装成结论。

**验证边界**：可控 HTTP fake 分别覆盖两个标题锚定槽位和无匹配标题的空计划。它只能锁定供应商
适配器请求契约；是否改善固定数据集中的 `ARF-026`，仍须在合并后经单独授权的真实供应商运行确认。

## 40. 二阶段细化丢弃标题锚点

**诊断修正**：在 `2abeb2b` 上运行的已授权两题真实供应商探针
`output/evals/issue91-2abeb2b-title-anchoring-probe.json`，SHA-256 为
`44c669da149cf7f7e3f72573f8c9b597a2a44b57797bbb740c811ecfe26c81e4`。`ARF-026` 的初始计划
输出和精化 disposition 当时没有写入轨迹；报告只能证明最终实际执行的是“环境动作 组件 论文”和
“critique token 组件 论文”。`89707e2` 上 v8 两题探针
`output/evals/issue93-89707e2-title-anchor-probe.json`，SHA-256 为
`3dbfd3e45e27457c5f75ead29a0813bf78dba874745d280f09be465c5a13494a`，也只记录同一执行路径。
因此不能据现有证据断言 v7 初始计划产生过锚点，也不能断言 v8 初始计划为空；两种情况均缺少
可审计的规划阶段证据。

**修复边界**：`two-stage-evidence-slots-v8` 将 `title_anchor` 作为每个规划槽位的显式结构字段。
它必须是无扩展名标题中至少三个字符的精确子串，并出现在初始和细化查询中；细化必须原样复制
当前槽位的标题和锚点。缺失、替换或不能嵌入查询的锚点使该细化槽位失效，不会改写为模型猜测或
跨槽位借用锚点。该字段仍只是检索假设，不增加查询预算、候选池、模型调用次数，也不构成回答
证据或来源归属。

**验证边界**：离线 HTTP fake 覆盖双槽位各自保留锚点、无效初始锚点的一次纠正，以及细化缺失
锚点时仅丢弃该槽位。它不证明真实模型语义效果；任何新的真实供应商探针必须在合并后单独授权，
并使用新的输出路径，不改写既有报告。

## 41. 规划阶段缺少去敏诊断轨迹

**根因**：`RetrievalPlanProposal` 只向检索服务传递最终附加查询，Answer Run 和 Evaluation
Decision Trace 只保存实际执行的查询。空计划、一次纠正后的接受或拒绝、以及每个精化槽位的
丢弃原因均在模型适配器边界被丢失。该缺口使两份 `ARF-026` 探针无法区分模型返回空计划、初始
结构无效或精化失败，继续修改提示词将缺乏可证伪的诊断依据。

**修复边界**：规划器为每次完成或初始规划阶段因供应商/结构错误失败的计划尝试生成有界、去敏的值对象，记录初始 disposition、
是否使用一次纠正、初始槽位数量与最终选中槽位的标题锚点、精化 disposition。它不保存原始 JSON、
完整标题列表、提示词、文档正文、评测真值或未执行查询，不改变规划、检索、证据或拒答行为。
历史记录的 `planning` 为 `null`，表示未知而非回填推断。

**验证边界**：离线 fake 锁定空计划、一次纠正后的接受、双槽位精化成功和锚点变更/缺失丢弃，
并验证工作流、历史 API 和评测观察可以携带同一去敏轨迹。该实现不调用真实供应商；新轨迹只有
在后续经单独授权的新运行中才能用于诊断 `ARF-026`。

## 42. 两题规划诊断缺少受控、非评分的执行入口

**根因**：真实 `evaluation.cli real` 只接受整个数据集并继续执行检索、证据判断、生成和评分。即使
诊断目标只是 `ARF-026` 的规划阶段，也会产生完整 30 题供应商成本，并把规划问题与下游门禁混在同一
报告中。

**修复边界**：新增 `planning-probe` 命令，只接受一到两个显式、已审核的 Dataset case，复用当前
数据集快照内可检索标题和生产 `QuestionPlanner`，在检索前停止。去敏工件仅保存 case ID、终态、
运行溯源和规划轨迹；不保存问题、参考答案、模型回答、查询、提示词、完整标题列表、文档正文、
Chunk ID、证据或分数。规划错误是非评分失败观察，不能伪装为拒答或 Evaluation Report。

**验证边界**：离线 fake 覆盖 case 数量/ID 边界、成功与失败轨迹、CLI 显式确认和工件去敏；本轮不调用
真实供应商、不修改 Dataset、检索策略、证据门禁或历史评测结果。实际 `ARF-025`/`ARF-026` 探针仍须在
合并后单独授权，且只能用于形成后续可证伪假设，不能宣称产品质量改善。
