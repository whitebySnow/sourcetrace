...
**修复**：新增窄语言策略。问题包含汉字时目标语言确定为中文；不含汉字但包含拉丁字母时确定
为英文；无法从字符集判断时不猜测。生成、Citation Repair 和声明支持度提示词收到同一明确
目标语言，并要求只翻译证据支持的含义、不得借翻译增加解释性细节。声明支持度和最终 Citation
校验之后新增确定性终态门禁：剥离标准 Citation 标签后，每个回答声明必须匹配目标字符集，
