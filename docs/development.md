...
自身文本分别调用 BGE reranker；原始问题最多保留 4 个查询覆盖候选，每条模型查询最多保留
1 个，再按最高逐查询 reranker 分数、融合分数、最佳原始余弦分数和 chunk UUID 补足最终排序。
...
`ANSWER_WORKFLOW_VERSION=langgraph-bounded-multi-query-v6`，用于重放时识别完整决策配置。
