...
供应商 timeout 分为四层：`LLM_CONNECT_TIMEOUT_SECONDS` 限制建连与连接池等待，
`LLM_READ_TIMEOUT_SECONDS` 限制两次网络数据之间的等待，`LLM_REQUEST_TIMEOUT_SECONDS` 限制
一次完整请求 lifecycle，`LLM_OPERATION_DEADLINE_SECONDS` 限制包含一次有界重试及退避的整个
模型操作。总 deadline 必须至少容纳两次 request timeout 与固定退避；默认分别为 10、120、
180 和 361 秒。供应商发送的 SSE 或空行 keep-alive 可以推进底层 read，但不能无限延长单次
request 或整个操作；超时统一映射为 `LLM_TIMEOUT`。旧 `LLM_TIMEOUT_SECONDS` 已废弃，升级时
应删除并改用这四项配置。
...
非流式结构化调用遇到连接、单次 request lifecycle timeout 或协议级瞬时异常时也会在同一总
deadline 内最多重试一次；HTTP 状态错误
...
限制或失败模式的绝对化、否定性声明生成零或一个反例查询；简单事实、对比、归属和组合题不在
初始阶段扩展。证据不足后仍可补检索一次，整个 Answer Run 的附加查询总预算仍为两个，
规范化后重复的查询不会执行。规划器只接收当前知识库最多 50 个稳定排序的可检索文档标题；
标题用于约束检索假设，不是回答证据，规划阶段不发送文档正文。
...
Reciprocal Rank Fusion 合并，`RETRIEVAL_RRF_RANK_CONSTANT` 配置融合排名常数。每个查询用
自身文本分别调用 BGE reranker；原始问题最多保留 4 个查询覆盖候选，所有覆盖配额之和不超过
实际 Top-K。容量允许时，每个实际有候选的查询按输入顺序先保留一个，再优先补足原始问题，最后按额外槽位顺序补第二个
候选，每槽至多 2 个；默认两个额外槽位时为 4、2、2。再按最高逐查询 reranker 分数、融合分数、
最佳原始余弦分数和 chunk UUID 补足最终排序。
逐查询分数、排名和覆盖标记都会写入轨迹；原始余弦分数仍用于最低证据阈值。
...
`ANSWER_WORKFLOW_VERSION=langgraph-bounded-multi-query-v8`，用于重放时识别完整决策配置。
...
禁止把旧结论套到新一轮模型输出。
供应商或模型基础设施错误会终止评测，不能被统计为回答失败。若命令已进入真实运行，CLI 会在
正常报告相邻路径写入去敏的 `-failure.json` 工件；它只包含数据集与已知运行元数据、失败 case ID、
工作流阶段和安全错误分类，不包含题目、回答、提示词、证据正文或任何分数，且不能传给 `review`
