...
async def test_slot_coverage_preserves_channel_leader_when_reranker_disagrees() -> None:
    original = [
        _evidence(
            f"00000000-0000-0000-0000-00000000003{index}",
            score=0.90 - index / 100,
            page_number=index,
        )
        for index in range(1, 5)
    ]
    channel_leader = _evidence(
        "00000000-0000-0000-0000-000000000035",
        score=0.80,
        page_number=5,
    )
    reranker_leader = _evidence(
        "00000000-0000-0000-0000-000000000036",
        score=0.79,
        page_number=6,
    )
    reranker_runner_up = _evidence(
        "00000000-0000-0000-0000-000000000037",
        score=0.78,
        page_number=7,
    )
    second_slot_first = _evidence(
        "00000000-0000-0000-0000-000000000038",
        score=0.77,
        page_number=8,
    )
    second_slot_second = _evidence(
    ...
    service = RetrievalService(
        repository=RankedListRepository(
            {
                (1.0,): original,
                (2.0,): [channel_leader, reranker_leader, reranker_runner_up],
                (3.0,): [second_slot_first, second_slot_second],
            }
        ),
        embedding_provider=RecordingEmbeddingProvider(((1.0,), (2.0,), (3.0,))),
        question_planner=StaticPlanner(),
        reranker=QuerySpecificReranker(
            {
                "original": {item.text: 0.90 - index / 100 for index, item in enumerate(original)},
                "first slot": {
                    channel_leader.text: 0.10,
                    reranker_leader.text: 0.99,
                    reranker_runner_up.text: 0.98,
                },
                "second slot": {
                    second_slot_first.text: 0.97,
                    second_slot_second.text: 0.96,
                },
            }
        ),
        top_k=8,
    ...
    assert selected_first_slot == {channel_leader.chunk_id, reranker_leader.chunk_id}
    assert reranker_runner_up not in result.primary_evidence
